...
// Helper function to fetch celebrations by date field
// Helper function to fetch celebrations by date field
// Helper function to fetch celebrations by date field
...
exports.getAllTodayCelebrations = async (req, res, next) => {
  ...
  const [birthdays, marriageAnniversaries, workAnniversaries, totalBirthdays, totalMarriageAnniversaries, totalWorkAnniversaries] = await Promise.all([
...
exports.getAllUpComingCelebrations = async (req, res, next) => {
  ...
  if (upcomingDays < 1 || upcomingDays > 30) {
    return next(new ErrorResponse('Upcoming days must be between 1 and 30', 400));
  }
  ...
  // Fetch upcoming celebrations only (excluding today)
    ...
    data: {
      upcoming: {
        birthdays: upcomingBirthdays.map(user => ({
          ...user,
          celebrationDate: moment().year(date.year()).month(moment(user.dateField).month()).date(moment(user.dateField).date()).format('YYYY-MM-DD'),
          daysUntil: moment().year(date.year()).month(moment(user.dateField).month()).date(moment(user.dateField).date()).diff(date, 'days')
        })),
  ...
  res.status(200).json({
    ...
    data: {
      ...
      upcoming: includeUpcoming
        ? upcoming.map(user => ({
          ...user,
          celebrationDate: moment().set({ month: moment(user.dateField).month(), day: moment(user.dateField).date() }).format('YYYY-MM-DD'),
          daysUntil: Math.ceil(moment(user.dateField).set('year', date.year()).diff(date, 'days'))
        }))
        : []
    }
  });
...
exports.getEmployeeDetails = async (req, res, next) => {
  ...
  const employee = await User.findOne({ employeeId, isActive: true })
    .select('employeeId firstName lastName email department dateOfBirth marriageAnniversary dateOfJoining maritalStatus spouseDetails fullName designation')
    .populate({ path: 'department', select: 'name' })
    .lean();

  if (!employee) {
    return next(new ErrorResponse(`Employee with ID ${employeeId} not found`, 404));
  }
